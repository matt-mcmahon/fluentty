...
export class Prompt {
    constructor({ message: message1 , retry: retry1 = false , defaultTo: defaultTo1 , sanitizers =[] , suggestions =[] , validators =[] , formatters =[]  }){
...
Deno.test("question.ts", async ()=>{
    const q = await question("This should partially match").acceptPartial("one", "two", "three").retry().IO();
    console.log(q);
});
const doIO = (prompt)=>stdout(`${prompt.message}: ${prompt.hint}`).then(stdin).then((input)=>prompt.validate(input)
    ).catch((reason)=>{
...
;
const insensitive = (matchStrategy)=>(input)=>(option)=>matchStrategy(option.toLocaleLowerCase())(input.toLocaleLowerCase())
;
const validateDefaultTo = (defaultTo1)=>(input)=>input === "" ? defaultTo1 : false
;
const setDefaultTo = (value)=>({ validators: current = [] , ...prompt })=>new Prompt({
            ...prompt,
            defaultTo: value,
            validators: [
                ...current,
                validateDefaultTo(value)
            ]
        })
;
...
;
const addSuggestions = (...additions)=>(prompt)=>{
...
;
const addPartialSuggestions = (...additions)=>(prompt)=>{
        const match = insensitive(partialMatch);
        const validate = (input)=>additions.find(match(input)) ?? false
        ;
        return new Prompt({
            ...prompt,
            suggestions: [
...
class Question {
    ...
    accept = (...suggestions1)=>Question.from(this.#prompt.then(addSuggestions(...suggestions1)))
    ;
    acceptPartial = (...additional)=>Question.from(this.#prompt.then(addPartialSuggestions(...additional)))
    ;
...
;
export const encodeText = (source)=>(message2)=>{
